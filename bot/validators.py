...
def validate_side(side: str) -> str:
    """Validate order side is either BUY or SELL."""
...
def validate_quantity(quantity: float) -> float:
    """Validate quantity is a float greater than 0."""
    try:
        q = float(quantity)
        if q <= 0:
            raise ValueError()
...
def validate_price(price: float, order_type: str) -> float:
    """Validate price is provided and > 0 if order is LIMIT."""
    if order_type == "LIMIT":
        if price is None:
...
def validate_stop_price(stop_price: float, order_type: str) -> float:
    """Validate stop_price is provided and > 0 if order is STOP_MARKET."""
